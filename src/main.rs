use anyhow::Context;
use rl_stats_ingester::run_connector;
use tokio::io;
...
fn start_runtime() -> anyhow::Result<tokio::runtime::Runtime> {
    ...
    // So map the debug and trace levels to a filter that will still use info level for other crates.
    let level_str = match std::env::var("LOG_LEVEL").ok() {
        Some(lvl) if lvl.as_str() == "trace" => {
            "rl_stats_ingester=trace,doc=debug,json=debug,info".to_string()
        }
        Some(lvl) if lvl.as_str() == "debug" => {
            "rl_stats_ingester=debug,doc=debug,json=debug,info".to_string()
        }
        Some(other) => other,
        None => "info".to_string(),
    };
